...
if (!empty($_POST)) {
  ...
  $idVehiculo = $mysqli->real_escape_string($_POST['idVehiculo']);


  if (isNullSV($idSocio, $idVehiculo)) {
    $errors[] = 'Debe asignar todos los campos';
  }

  if (noRepite($idVehiculo)) {
    $errors[] = 'El vehículo ya ha sido asignado';
  }

  if (count($errors) == 0) {

    $asignarVS = asignarVS($idSocio, $idVehiculo);
    if ($asignarVS > 0) {

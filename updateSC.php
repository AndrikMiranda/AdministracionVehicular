...
if (!empty($_POST)) {
  ...
  $idSocio = $mysqli->real_escape_string($_POST['idSocio']);

  if (isNullSC($idConductor, $idSocio)) {
    $errors[] = 'Debe asignar todos los campos';
  }

  if (noRepiteConductor($idConductor)) {
    $errors[] = 'El conductor ya ha sido asignado';
  }

  if (count($errors) == 0) {

    $asignarSC = asignarSC($idConductor, $idSocio);

    if ($asignarSC > 0) {
        $message = 'Se ha guardado en la base de datos';
    }  else {
      $errors[] = 'Ha ocurrido un problema intenta de nuevo';
    }
  }
